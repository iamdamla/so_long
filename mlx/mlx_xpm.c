...
char	*mlx_int_get_line(char *ptr, int *pos, int size)
{
	...
	if ((pos2 = mlx_int_str_str(ptr + *pos, "\"", size - *pos)) == -1)
		return ((char *)0);
	if ((pos3 = mlx_int_str_str(ptr + *pos + pos2 + 1, "\"",
					size - *pos - pos2 - 1)) == -1)
		return ((char *)0);
	*(ptr + *pos + pos2) = 0;
		...
		char *(*f)())
{
	...
	while (i--)
	{
		...
		{
			if (!(clip_data = malloc(4 * width * height)) ||
		...
		else
		{
			...
			colors[i].col = rgb_col >= 0 ? mlx_get_color_value(xvar, rgb_col)
				: rgb_col;
		}
		free(tab);
	}
...
void	*mlx_xpm_file_to_image(void *xvar, char *file, int *width, int *height)
{
	...
	if ((fd = open(file, O_RDONLY)) == -1 ||
			...
			(ptr = mmap(0, size, PROT_WRITE |
						PROT_READ, MAP_PRIVATE, fd, 0)) == (void *)MAP_FAILED)
	{
		if (fd >= 0)
			close(fd);
		return ((void *)0);
